...
class Extracton():
    def __init__(self,path='./data_match',file_name='match_1.json'):
        self.path=path
...
class Transform():
    ...
    def create_playstyle_sequences_for_lstm(self, playstyle_actions, playstyle_threshold=0.5, min_sequence_length=5):
    ...
    def regressor_data_construction(self):
        ...
        df_reg =df_reg.drop(df_reg[(df_reg['sequence_length'] < 10) | (df_reg['sequence_length']>300)].index) 
        ...
        df_reg['sample_temporal_index'] = df_reg.index-1
    ...
    def create_for_normvalues_data(self):
        ...
        df_norm =df_norm.drop(df_norm[(df_norm['sequence_length'] < 10) | (df_norm['sequence_length']>300)  ].index) 
...
class Load():
    ...
    def save_data(self):
        self.df.to_csv('loaded_data.csv', index=False)